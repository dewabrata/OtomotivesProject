    ...
    private void initRv() {
        ...
        rvBonus.setAdapter(new NikitaRecyclerAdapter(bonusList, R.layout.item_bonus_refferal) {
            ...
            public void onBindViewHolder(@NonNull final NikitaViewHolder viewHolder, @SuppressLint("RecyclerView") final int position) {
                ...
                viewHolder.find(R.id.tv_nama_kontak, TextView.class).setText(bonusList.get(position).get("").asString());
                viewHolder.find(R.id.tv_no_ponsel, TextView.class).setText(bonusList.get(position).get("").asString());
                viewHolder.find(R.id.tv_nama_bengkel, TextView.class).setText(bonusList.get(position).get("").asString());
                viewHolder.find(R.id.tv_tgl_aktif, TextView.class).setText(bonusList.get(position).get("").asString());
                viewHolder.find(R.id.tv_jumlah_fee, TextView.class).setText(bonusList.get(position).get("").asString());
                viewHolder.find(R.id.tv_jumlah_bonus, TextView.class).setText(bonusList.get(position).get("").asString());
                viewHolder.find(R.id.tv_tgl_transfer, TextView.class).setText(bonusList.get(position).get("").asString());
    ...
    private void viewList() {
        activity.newTask(new Messagebox.DoubleRunnable() {
            ...
            public void run() {
                ...
                args.put("", "");
                result = Nson.readJson(InternetX.postHttpConnection(AppApplication.getBaseUrlV3(BONUS_REFFERAL), args));
            }
